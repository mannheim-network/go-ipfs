...
func (b *arccache) Put(bl blocks.Block) error {
	if !spacex.IsWarpedSealedBlock(bl) {
	...
	if err == nil {
		if !spacex.IsWarpedSealedBlock(bl) {
			...
			_, sb := spacex.TryGetSealedBlock(bl.RawData())
...
func (b *arccache) PutMany(bs []blocks.Block) error {
	...
	for _, block := range bs {
		...
		if has, _, ok := b.hasCached(block.Cid()); !ok || (ok && !has) || spacex.IsWarpedSealedBlock(block) {
	...
	for _, block := range good {
		if !spacex.IsWarpedSealedBlock(block) {
			b.cacheSize(block.Cid(), len(block.RawData()))
		} else {
			_, sb := spacex.TryGetSealedBlock(block.RawData())
